...
doLrta s@(State (Grid g) p (Grid v) i) = case lrta p succsessor (i!) of
  Just (d,a) -> State (Grid g) a (Grid $ v//[(p,True)]) (i//[(p,traceR d)])
  Nothing -> s
  where
    succsessor (x,y) = filter (\p -> inBounds (bounds v) p && not (g!p)) [(x-1,y),(x+1,y),(x,y-1),(x,y+1)]
...
test = do
  g <- grid2
  simulate (InWindow "Yay" (200,200) (200,200)) white 2 (initState $ Grid g) drawState (\_ _ m -> doLrta m)
